...
enum Event {
    ...
    // New
    Written { slug: String, docid: String },
    Deleted { slug: String },
...
This ensures that if the daemon crashes after acking `Queued` but before completing the job, the job is replayed on restart. On daemon startup, scan for `status = 'pending'` or `status = 'processing'` (crashed mid-job) and re-queue them.
...
**Filesystem writes** (markdown files to `wiki/`) happen AFTER the SQLite transaction commits successfully. If the transaction fails, no files are written. If a file write fails after commit, the SQLite state is ahead of the filesystem, but `memex lint` can detect and repair this (the document exists in SQLite but the file is missing).
...
The daemon handles all storage: content insertion, document upsert, embedding, backlink updates, source document storage (for `--source`).

**Escape hatch:** `memex write --direct` bypasses the daemon and writes directly to SQLite with cold ONNX. Not the default, not advertised. For emergencies and development when the daemon is broken. The direct write codepath is the existing implementation, kept behind a flag.
...
- `--direct` available as break-glass option
