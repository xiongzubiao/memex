//! Integration test for the gemini worker's synth path.
//!
//! Symlinks `mock-gemini.sh` as `gemini` on PATH and sets
//! `MEMEX__DAEMON__WORKER__AGENT=gemini` so the daemon spawns the mock.
...
fn mock_gemini_on_path() -> (TempDir, PathBuf) {
    let tmp = TempDir::new().unwrap();
    let bin_dir = tmp.path().to_path_buf();
    let fixture = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/mock-gemini.sh");
    assert!(fixture.exists(), "mock fixture missing: {fixture:?}");
    std::os::unix::fs::symlink(&fixture, bin_dir.join("gemini")).unwrap();
    (tmp, bin_dir)
}
...
fn memex_cmd(root: &Path, extra_path: &Path) -> Command {
    ...
    cmd.env("MEMEX__DAEMON__WORKER__AGENT", "gemini");
...
fn gemini_synth_returns_answer_with_citation() {
    let (_mock, extra_path) = mock_gemini_on_path();
    let tmp = TempDir::new().unwrap();
