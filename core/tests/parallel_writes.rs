...
fn two_writers_serialize() {
    ...
    assert!(
        ...
        "expected B to wait behind A, waited only {:?}",
        start.elapsed()
    );
...
fn writer_timeout_returns_correct_error() {
    ...
    assert!(matches!(
        result,
        Err(memex_core::error::MemexError::LockTimeout { .. })
    ));
}
...
fn concurrent_readers_ok_while_writer_holds() {
    ...
    for _ in 0..10 {
        ...
        handles.push(std::thread::spawn(move || {
            ...
            memex
                .search()
                .search_collection("anything", "wiki", 5)
                .unwrap()
        }));
    }
    for h in handles {
...
fn lock_acquire_io_preserves_error_taxonomy() {
    ...
    #[cfg(unix)]
    {
        ...
        assert!(
            matches!(
                result,
                Err(memex_core::error::MemexError::LockAcquireIo { .. })
            ),
            "expected LockAcquireIo, got {:?}",
            result
        );
